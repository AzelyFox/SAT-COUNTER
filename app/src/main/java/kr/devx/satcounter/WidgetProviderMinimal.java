...
public class WidgetProviderMinimal extends AppWidgetProvider {
    ...
    private void updateAppWidget(Context context, AppWidgetManager widgetManager, int id) {
        RemoteViews view = new RemoteViews(context.getPackageName(), R.layout.widget_minimal);
        ...
        view.setTextViewText(R.id.widget_minimal_text, context.getString(R.string.main_general_dayleft_prefix) + DayUtil.getDaysLeft(userData.getUserDate()));
        if (WIDGET_COLOR_WHITE == 1) {
            view.setTextColor(R.id.widget_minimal_text, ContextCompat.getColor(context, R.color.colorWhite));
        }
