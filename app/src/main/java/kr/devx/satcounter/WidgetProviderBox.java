...
public class WidgetProviderBox extends AppWidgetProvider {
    ...
    private void updateAppWidget(Context context, AppWidgetManager widgetManager, int id) {
        RemoteViews view = new RemoteViews(context.getPackageName(), R.layout.widget_box);
        ...
        view.setTextViewText(R.id.widget_box_bodyText, context.getString(R.string.main_general_dayleft_prefix) + DayUtil.getDaysLeft(userData.getUserDate()));
        if (satApplication.userUniversity != null) {
            view.setImageViewResource(R.id.widget_box_image, satApplication.userUniversity.getLogoDrawable());
            view.setTextViewText(R.id.widget_box_headerText, satApplication.userUniversity.getName());
        } else {
            view.setViewVisibility(R.id.widget_box_image, View.GONE);
            view.setTextViewText(R.id.widget_box_headerText, userData.getUserGoal());
        }
        if (!TextUtils.isEmpty(userData.getUserGoalOptional())) {
            view.setTextViewText(R.id.widget_box_footerText, userData.getUserGoalOptional());
        } else {
            view.setTextViewText(R.id.widget_box_footerText, satApplication.randomFromWordList());
        }
        if (WIDGET_COLOR_WHITE == 1) {
            view.setTextColor(R.id.widget_box_headerText, ContextCompat.getColor(context, R.color.colorWhite));
            view.setTextColor(R.id.widget_box_bodyText, ContextCompat.getColor(context, R.color.colorWhite));
            view.setTextColor(R.id.widget_box_footerText, ContextCompat.getColor(context, R.color.colorWhite));
        }
